...
private:
    ...
    vector<enemy> en;
    // il vettore expAnimation di un vettore di puntatori void. Conterrà tutti i frame dell'animazione di esplosione
    void** expAnimation;
...
bool Enemies::collide(int left,int top, int right, int bottom, bool rem, Player& pl, int wave) {
    ...
    for(int i = 0; i < num;i++) {
        ...
        if(col) {
            ...
            // dato che è impossibile che un proiettile colpisca 2 navicelle (idealmente) e che non è importante se il giocatore viene colpito da 2 navicelle contemporaneamnete, non serve cercare altre collisioni
            break;
        }
    }
    return col;
